...
const createTodo = async (todo) => {
  let newTodo;
  try {
    newTodo = await db.one(`INSERT INTO todos(owner, text) 
      VALUES($/owner/, $/text/) RETURNING *`, todo)
    return newTodo;
  } catch (err) {
    if (err.code === "23503") { // Owner for todo doesn't exists
      throw new Error("Owner for todo doesn't exist.")
    }
    throw err;
  }
}
